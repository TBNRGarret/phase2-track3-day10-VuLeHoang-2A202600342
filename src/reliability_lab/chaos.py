...
def run_simulation(config: LabConfig, queries: list[str]) -> RunMetrics:
    """Run all named scenarios from config with pass/fail validation."""
    ...
    for scenario in config.scenarios:
        ...
        passed = False
        availability = result.successful_requests / result.total_requests if result.total_requests > 0 else 0
        
        if scenario.name == "primary_timeout_100":
            # Pass if system stayed alive via fallback despite primary being dead
            passed = availability > 0.9 and result.fallback_successes > 0
        elif scenario.name == "primary_flaky_50":
            # Pass if circuit breaker actually opened at least once to protect system
            passed = result.circuit_open_count > 0 and availability > 0.4
        elif scenario.name == "all_healthy":
            # Pass if availability is very high
